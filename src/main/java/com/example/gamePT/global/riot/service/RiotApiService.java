...
import com.example.gamePT.global.riot.entity.ChampionDTO;
...
public class RiotApiService {
    ...
    private String summonerApi = "https://kr.api.riotgames.com/lol/summoner/v4/summoners/by-name";
    ...
    public ChampionDTO getChampion(int key) {
        try {
            String encodedName = URLEncoder.encode(key, "UTF-8"); //공백을 처리하기 위해 인코딩
            String url = String.format("%s/%s?api_key=%s", summonerApi, encodedName, myKey);
            URL apiUrl = new URL(url);
            HttpURLConnection connection = (HttpURLConnection) apiUrl.openConnection();
            connection.setRequestMethod("GET");
